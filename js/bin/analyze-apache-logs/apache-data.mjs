...
class ApacheData {
    ...
    _printSingle(users, spams, title, usersText, spamsText, from = false, print = (size) => size.toString()) {
    ...
    print(options) {
        ...
        const computeSize = (logs) => logs.reduce((partialSum, log) => {
    ...
    addSpamIps(newSpamIps) {
        ...
        this.userIps = this.userIps.filter(ip => (ip !== '0.0.0.0') && (!newSpamIps.some(spamip => (ip === spamip))));
