...
function _checkLog(apacheData, requests, botsOnly, logField, textReason, fctToCheck) {
    ...
    if (requests.some(r => {
        if (configTexts.some(configText => { let logText = r[logField]; return fctToCheck(logText, configText); })) {
            return true;
        }
        else {
            return false;
...
async function spamDetection(apacheData, options) {
    ...
    const results = apacheData.userIps.map(ip => {
        ...
        spam = _checkLog(apacheData, requests, options.config.local.get.botsOnly, 'request', 'Accessing a file indicating a bot or a spammer', (logText, configText) => logText.startsWith('GET /' + configText));
        if (spam !== undefined) {
            return spam;
        }
        // check forbidden keywords in user agent
        spam = _checkLog(apacheData, requests, options.config.local.userAgent.botsOnly, 'RequestHeader User-Agent', 'User Agent indicates a bot or a spammer', (logText, configText) => logText.toLowerCase().includes(configText.toLowerCase()));
        if (spam !== undefined) {
            return spam;
        }
